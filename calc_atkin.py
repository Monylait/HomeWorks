...
def Atkins(limit: int, s1: int, s2: int, s3: int):
    ...
    sieve = [False] * (limit + 1)
    check=0
    for x in range(1, int(math.sqrt(limit)) + 1,tik):
        for y in range(1, int(math.sqrt(limit)) + 1,tik):
            n = 4 * x ** 2 + y ** 2
            ...
            if check==100000:
                ...
                check=0
            check+=1
    for x in range(5, int(math.sqrt(limit)),tik):
        if sieve[x]:
...
if __name__ == '__main__':
    try:
        if int(argv[1])==0 or int(argv[1])>0 or int(argv[1])<0:
            flag=False
        else:
            raise Exception
        limit=int(argv[1])
        ...
        print("Pleas, dont press CTRL+C more")